  ...
  return (
    <div className="h-auto w-full p-3 bg-[#FFF5E5]">
      ...
      <div className="px-5 mb-8 flex justify-between items-center">
        ...
        <span className="text-sm uppercase nav cursor-pointer">View All →</span>
      </div>
      {/* GRID */}
      <div className="relative mb-10 px-5 grid grid-cols-1 sm:grid-cols-2 gap-6 w-full">
        {dummyMatches.map((item, i) => (
          <div key={i} className="w-full flex flex-col">
            ...
            <div className="mt-3 flex flex-col items-center justify-between sm:flex-row gap-3">
              <h4 className="nav text-xl sm:text-2xl font-medium uppercase tracking-wide text-center sm:text-left">
                {truncate(item.title)}
              </h4>

              <div className="flex text-sm sm:text-base gap-2">
                <span className="uppercase nav border px-2 py-1 rounded">
                  {item.tag}
                </span>
                <span className="nav border px-2 py-1 rounded">
                  {item.year}
                </span>
              </div>
            </div>
